...
namespace Circle.Code.Concepts
{
    ...
    /// ... Getting a property value is postponed as much as possible to prevent auto-instantiation
    /// of irrelevant properties.
    /// </summary>
    public class Response
    {
        // TODO: Response can not traverse lists yet.
        ...
        private void SetFieldOrProperty(object parent, MemberInfo member, HashSet<object> done)
        {
            ...
            MatchMember(parent, member, out value, out mustBind, out mustFollow);

            if (mustBind) JoinPoints.Add(value);

            if (mustFollow) SetFieldsAndProperties(value, done); // Recursive call
        }
        ...
        private void AnnullFieldOrProperty(object parent, MemberInfo member, HashSet<object> done)
        {
            ...
            if (mustBind) JoinPoints.Remove(value);

            if (mustFollow) AnnullFieldsAndProperties(value, done); // Recursive call
        }

        // Shared by Set & Annull

        private void MatchMember(object parent, MemberInfo member, out object value, out bool mustBind, out bool mustFollow)
        {
            value = null;
            mustBind = false;
            mustFollow = false;
            ...
            // Check is type to follow
            bool isTypeToFollow = false;
            if (!isParentMatch)
            {
                isTypeToFollow = IsTypeToFollow(parentType);
            }

            // Match name
            bool isMemberMatch = IsMemberMatch(member.Name);
            bool isMemberExcluded = IsMemberExcluded(member.Name);

            // Getting the value was postponed as much as possible 
            // to prevent auto-instatiation of irrelevant properties.

            if (isParentMatch || isTypeToFollow || (isMemberMatch & !isMemberExcluded))
            {
                // Never check IsCreated to prevent auto-instantiation,
                // because even when it is not created, you still have to unbind OnLive.

                value = GetValue(parent, member);
                ...
                if (isParentMatch)
                {
                    if (isMemberMatch & !isMemberExcluded)
                    {
                ...
                // Match types to follow
                if (isParentMatch || isTypeToFollow)
                {
                    mustFollow = true;
                }
            }
